...
public class SwiftAmplifyDataStorePlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "deleteInstance":
             onDeleteInstance(args: arguments, flutterResult: result)
        default:
    ...
    private func onDeleteInstance(args: [String: AnyObject], flutterResult: @escaping FlutterResult) {
        
        do {
            let modelName = args["modelName"] as! String

            let modelData = SerializedModel(map: getJSONValue(args["model"] as! [String : Any]))
            let queryPredicates = try QueryPredicateBuilder.fromSerializedMap(serializedMap: args["queryPredicate"] as? [String : AnyObject])
            
            getPlugin().delete(modelData, modelSchema: flutterModelRegistration.modelSchemas[modelName]!, where: queryPredicates) { (result) in
                switch result {
                    case .success(let res):
                        let serializedResults = res.map { (queryResult) -> [String: Any] in
                            return queryResult.toJSON()
